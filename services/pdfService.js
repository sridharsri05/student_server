...
const drawHeader = (doc, title) => {
    ...
    // Draw stylized "E" for EduFlow
    doc.fillColor('white');
    
    // Horizontal lines of the E
    doc.rect(8, 8, 24, 5).fill();
    doc.rect(8, 18, 20, 4).fill();
    doc.rect(8, 28, 24, 5).fill();
    
    // Vertical line of the E
    doc.rect(8, 8, 5, 25).fill();
...
const createTable = (doc, headers, rows, startX, startY, colWidths, alignments = []) => {
    const rowHeight = 25;
...
export const generateInvoicePDF = async (student, payment) => {
    try {
        ...
        // Use right alignment for the amount column
        let yPos = createTable(doc, headers, rows, 50, 230, [350, 150], ['left', 'right']);
        ...
        doc.text(`₹${((payment.totalAmount || 0) - (payment.depositAmount || 0)).toLocaleString()}`, 
            480, yPos + 30, { align: 'right', width: 10 });
        ...
        if (payment.installments?.length > 0) {
            ...
            // Use right alignment for the amount column
            createTable(doc, installmentHeaders, installmentRows, 50, yPos + 100, [100, 150, 100, 100], ['left', 'left', 'right', 'left']);
        }
        ...
        doc.text('Thank you for choosing EduFlow!', 0, doc.page.height - 25, {
            align: 'center',
...
export const generatePaymentReportPDF = async (payments, analytics, filters = {}) => {
    try {
        ...
        // Use right alignment for the amount column
        createTable(doc, headers, rows, 50, startY + boxHeight + 50, [150, 100, 100, 80, 100], ['left', 'right', 'left', 'left', 'left']);
        ...
        doc.fillColor('white').fontSize(10).font('Helvetica');
        doc.text('EduFlow - Confidential', 0, doc.page.height - 25, {
            align: 'center',
            width: doc.page.width
        });
